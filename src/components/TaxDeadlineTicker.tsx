...
export const TaxDeadlineTicker: React.FC<TaxDeadlineTickerProps> = ({ deadlines, clients = [] }) => {
        ...
        {visibleDeadlines.length === 0 ? (
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-1 md:pb-0 scrollbar-none">
            {visibleDeadlines.map((item) => {
              ...
              const linkedClientName = clientNameFor(item.clientId);
              return (
                <div
                  key={item.id}
                ...
                >
                  ...
                  {linkedClientName && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-indigo-500/20 text-indigo-200 px-1.5 py-0.5 rounded-full border border-indigo-400/30 shrink-0">
